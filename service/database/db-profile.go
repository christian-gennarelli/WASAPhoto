...
func (db appdbimpl) GetUserProfile(Username string) (*components.Profile, error) {
	...
	stmt, err := db.c.Prepare("SELECT Username, COALESCE(Birthdate, ''), COALESCE(Name, '') FROM User WHERE Username = ?")
	if err != nil {
	...
	if err = stmt.QueryRow(Username).Scan(&User.Username, &User.Birthdate, &User.Name); err != nil {
